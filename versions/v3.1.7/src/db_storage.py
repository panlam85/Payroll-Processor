...
from app_paths import CONFIG_DIR, CONFIG_PATH, PREFS_PATH
...
class _LazyPandas:
    """Import pandas only when a dataframe operation is requested."""
...
def get_connection(config: Dict[str, object]):
    ...
    return psycopg2.connect(
        ...
        connect_timeout=max(1, int(config.get("connect_timeout") or 3)),
    )
