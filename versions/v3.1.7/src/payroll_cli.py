...
def run_processing(args: argparse.Namespace) -> int:
    ...
    artifact_id = f"{timestamp}_{safe_run_id}_{uuid.uuid4().hex[:8]}"
    summary_output = output_dir / f"{args.report_prefix}_{artifact_id}_summary.xlsx"
    detail_output = output_dir / f"{args.report_prefix}_{artifact_id}_detail.xlsx"
    ...
    if not csv_files:
        finished = _utc_now()
        had_errors = bool(ledger_entry["errors"])
        ledger_entry["status"] = "error" if had_errors else "no-data"
        ledger_entry["finished_at"] = _isoformat(finished)
        ledger_entry["finished_at_epoch"] = finished.timestamp()
        _write_ledger(ledger_entry, Path(args.ledger_dir) if args.ledger_dir else DEFAULT_LEDGER_DIR)
    ...
    summary_temp = output_dir / f".{summary_output.name}.{temp_token}.tmp.xlsx"
    detail_temp = output_dir / f".{detail_output.name}.{temp_token}.tmp.xlsx"
    try:
        create_employee_reports.write_employee_reports(summary_df, str(summary_temp))
        create_employee_reports.write_detail_report(combined_df, str(detail_temp))
        os.replace(summary_temp, summary_output)
        os.replace(detail_temp, detail_output)
    except Exception as exc:
        for temp_path in (summary_temp, detail_temp):
            try:
                temp_path.unlink()
            except FileNotFoundError:
                pass
        ledger_entry["errors"].append(f"Report generation failed: {exc}")
        finished = _utc_now()
        ledger_entry["status"] = "error"
        ledger_entry["finished_at"] = _isoformat(finished)
        ledger_entry["finished_at_epoch"] = finished.timestamp()
        _write_ledger(ledger_entry, Path(args.ledger_dir) if args.ledger_dir else DEFAULT_LEDGER_DIR)
    ...
    return 1 if ledger_entry["errors"] else 0
