...
# Payroll parsing and charting are the two heaviest import stacks in the app.
# Keep first paint lightweight and load each stack only when its feature is
# actually opened or used.
process_payroll = None
create_employee_reports = None
pd = None
...
def _load_processing_stack():
    global process_payroll, create_employee_reports, pd
    if process_payroll is None:
        import process_payroll as payroll_module
        process_payroll = payroll_module
    if create_employee_reports is None:
        import create_employee_reports as report_module
        create_employee_reports = report_module
    if pd is None:
...
def _load_chart_stack():
    global FigureCanvasTkAgg, NavigationToolbar2Tk, Figure, FuncFormatter, plt
    if Figure is None:
        from matplotlib.backends.backend_tkagg import FigureCanvasTkAgg as canvas_class
        from matplotlib.backends.backend_tkagg import NavigationToolbar2Tk as toolbar_class
        from matplotlib.figure import Figure as figure_class
        from matplotlib.ticker import FuncFormatter as formatter_class
        import matplotlib.pyplot as pyplot_module

        FigureCanvasTkAgg = canvas_class
        NavigationToolbar2Tk = toolbar_class
        Figure = figure_class
        FuncFormatter = formatter_class
        plt = pyplot_module
...
class PayrollProcessorGUI:
    ...
    def __init__(self):
        ...
        self.root.after(100, self._initialize_database_schema_async)
        self.root.after(250, self._load_app_icon)
    ...
    def _initialize_database_schema_async(self):
        """Run optional schema maintenance without holding up the window."""
        ...
        self._run_async("database-schema", _work, lambda _result: None, lambda _error: None)
    ...
    def configure_styles(self):
        ...
        _theme_mode, tokens = self.resolve_theme()
        self.theme = tokens
        ...
        self.style = style
        try:
            # App-owned chrome needs predictable colours and spacing in both
            # appearances; Aqua ignores most of those style options.
            style.theme_use("clam")
        except tk.TclError:
            pass
        self._is_aqua = False
        style.configure(".", font=(tokens.font_base, 12))
        style.configure("App.TFrame", background=tokens.bg)
        style.configure("Surface.TFrame", background=tokens.surface)
        style.configure("Raised.TFrame", background=tokens.surface_raised)
        style.configure("Sidebar.TFrame", background=tokens.sidebar_bg)
        style.configure(
            "Card.TFrame",
        ...
        style.configure("CardTitle.TLabel", background=tokens.surface, foreground=tokens.text_secondary, font=(tokens.font_base, 11, "bold"))
        style.configure("CardValue.TLabel", background=tokens.surface, foreground=tokens.text_primary, font=(tokens.font_display, 21, "bold"))
        ...
        style.configure("Header.TLabel", background=tokens.bg, foreground=tokens.text_primary, font=(tokens.font_display, 24, "bold"))
        style.configure("Section.TLabel", background=tokens.bg, foreground=tokens.text_primary, font=(tokens.font_display, 15, "bold"))
        style.configure("Body.TLabel", background=tokens.bg, foreground=tokens.text_secondary, font=(tokens.font_base, 12))
        style.configure("Hint.TLabel", background=tokens.bg, foreground=tokens.muted, font=(tokens.font_base, 11))
        style.configure("Warning.TLabel", background=tokens.bg, foreground=tokens.warning, font=(tokens.font_base, 12))
        style.configure("SurfaceTitle.TLabel", background=tokens.surface, foreground=tokens.text_primary, font=(tokens.font_display, 16, "bold"))
        ...
        style.configure("RunRail.TFrame", background=tokens.surface_raised)
        style.configure("RunStep.TLabel", background=tokens.surface_raised, foreground=tokens.text_secondary, font=(tokens.font_mono, 10, "bold"))
        style.configure("RunStepActive.TLabel", background=tokens.accent_soft, foreground=tokens.accent, font=(tokens.font_mono, 10, "bold"), padding=(10, 7))
        style.configure("RunConnector.TLabel", background=tokens.surface_raised, foreground=tokens.border, font=(tokens.font_mono, 10))
        style.configure("Status.TLabel", background=tokens.surface, foreground=tokens.accent, font=(tokens.font_mono, 10, "bold"))
        ...
        style.configure("TButton", padding=(12, 8), background=tokens.surface, foreground=tokens.text_primary, bordercolor=tokens.border, focuscolor=tokens.accent)
        style.map("TButton", background=[("active", tokens.hover), ("pressed", tokens.selection), ("disabled", tokens.surface_raised)], foreground=[("disabled", tokens.muted)])
        for field_style in ("TEntry", "TCombobox", "TSpinbox"):
            style.configure(field_style, padding=(8, 7), fieldbackground=tokens.field_bg, background=tokens.field_bg, foreground=tokens.text_primary, bordercolor=tokens.border, arrowcolor=tokens.text_secondary)
            style.map(field_style, bordercolor=[("focus", tokens.accent)], fieldbackground=[("readonly", tokens.field_bg)])
        style.configure("TCheckbutton", background=tokens.bg, foreground=tokens.text_primary, padding=(2, 5))
        style.map("TCheckbutton", background=[("active", tokens.bg)])
        style.configure("TProgressbar", background=tokens.accent, troughcolor=tokens.surface_raised, bordercolor=tokens.surface_raised, lightcolor=tokens.accent, darkcolor=tokens.accent)
        ...
        style.configure("Treeview.Heading", background=tokens.surface_raised, foreground=tokens.text_primary, font=(tokens.font_base, 11, "bold"), padding=(8, 7))
    ...
    def _configure_button_styles(self, style, tokens):
        ...
        style.configure(
            "Sidebar.TButton",
            padding=(14, 10),
            font=(tokens.font_base, 12),
            anchor=tk.W,
            background=tokens.sidebar_bg,
            foreground=tokens.sidebar_text,
            bordercolor=tokens.sidebar_bg,
            focuscolor=tokens.sidebar_bg,
            borderwidth=0,
            relief="flat",
        )
        style.map(
            "Sidebar.TButton",
            background=[("active", tokens.sidebar_hover), ("pressed", tokens.sidebar_hover)],
            foreground=[("active", tokens.sidebar_text), ("pressed", tokens.sidebar_text)],
        )
        style.configure(
        ...
        style.layout("Sidebar.TButton", sidebar_layout)
        style.layout("SidebarActive.TButton", sidebar_layout)
    ...
    def _load_app_icon(self):
        """Apply only the window icon after first paint.

        Navigation now uses typography instead of decoding a folder of large
        raster assets at startup. The installed app still keeps its branded
    ...
    def create_widgets(self):
        ...
        ttk.Label(sidebar, text="PAYROLL", style="SidebarBrand.TLabel").grid(row=0, column=0, sticky=tk.W)
        ttk.Label(sidebar, text=f"PROCESSOR  /  {APP_VERSION}", style="SidebarVersion.TLabel").grid(
            row=1, column=0, sticky=tk.W, pady=(1, 24)
        )
        nav_groups = (
            ("PRIMARY", (("Processing", "Process payroll"),)),
            ("RECORDS", (("Dashboard", "Overview"), ("Employees", "Employees"), ("Insurance", "Insurance"))),
            ("INSIGHTS", (("Analytics Data Grid", "Data"), ("Analytics Graphs", "Charts"))),
            ("SYSTEM", (("Database", "Database"), ("Settings", "Settings"))),
        )
        shortcut = 0
        row = 2
        for group_name, items in nav_groups:
            ttk.Label(sidebar, text=group_name, style="SidebarSection.TLabel").grid(
                row=row, column=0, sticky=tk.W, pady=(0 if row == 2 else 15, 5), padx=(4, 0)
            )
            row += 1
            for view_name, label in items:
                ...
                btn.grid(row=row, column=0, sticky=(tk.W, tk.E), pady=(0, 3))
                row += 1
        ...
        self.analytics_tab = ttk.Frame(self.notebook, padding="24", style="App.TFrame")
        self.analytics_grid_view_tab = ttk.Frame(self.notebook, padding="24", style="App.TFrame")
        self.insurance_tab = ttk.Frame(self.notebook, padding="24", style="App.TFrame")
        self.employees_tab = ttk.Frame(self.notebook, padding="24", style="App.TFrame")
        self.settings_tab = ttk.Frame(self.notebook, padding="24", style="App.TFrame")
        self.dashboard_tab = ttk.Frame(self.notebook, padding="24", style="App.TFrame")
        ...
        # The processing screen is the product's primary workspace.
        header = ttk.Frame(self.processing_tab, style="App.TFrame")
        header.grid(row=0, column=0, pady=(0, 18), sticky=tk.EW)
        title_block = ttk.Frame(header, style="App.TFrame")
        title_block.pack(side=tk.LEFT, anchor=tk.W)
        ttk.Label(title_block, text="PROCESSING RUN", style="Eyebrow.TLabel").pack(anchor=tk.W)
        ttk.Label(title_block, text="Process payroll", style="Header.TLabel").pack(anchor=tk.W, pady=(3, 0))
        self.instructions_label = ttk.Label(
            title_block,
            text="Add source files, review readiness, then create the summary and detail workbooks.",
            justify=tk.LEFT,
            wraplength=620,
            style="Body.TLabel",
        )
        self.instructions_label.pack(anchor=tk.W, pady=(5, 0))

        run_rail = ttk.Frame(self.processing_tab, style="RunRail.TFrame", padding=(14, 10))
        run_rail.grid(row=1, column=0, sticky=(tk.W, tk.E), pady=(0, 16))
        for column in (0, 2, 4):
            run_rail.columnconfigure(column, weight=1)
        self.run_stage_labels = []
        source_stage = ttk.Label(run_rail, text="01  SOURCES", style="RunStepActive.TLabel")
        source_stage.grid(row=0, column=0, sticky=tk.W)
        self.run_stage_labels.append(source_stage)
        ttk.Label(run_rail, text="────────", style="RunConnector.TLabel").grid(row=0, column=1, padx=10)
        check_stage = ttk.Label(run_rail, text="02  CHECK", style="RunStep.TLabel")
        check_stage.grid(row=0, column=2)
        self.run_stage_labels.append(check_stage)
        ttk.Label(run_rail, text="────────", style="RunConnector.TLabel").grid(row=0, column=3, padx=10)
        report_stage = ttk.Label(run_rail, text="03  REPORTS", style="RunStep.TLabel")
        report_stage.grid(row=0, column=4, sticky=tk.E)
        self.run_stage_labels.append(report_stage)
        ...
        # Source files
        list_frame = ttk.Frame(self.processing_tab, padding=16, style="Card.TFrame")
        list_frame.grid(row=3, column=0, sticky=(tk.W, tk.E, tk.N, tk.S), pady=(0, 12))
        list_frame.columnconfigure(0, weight=1)
        list_frame.rowconfigure(1, weight=1)

        source_heading = ttk.Frame(list_frame, style="Surface.TFrame")
        source_heading.grid(row=0, column=0, sticky=(tk.W, tk.E), pady=(0, 12))
        source_heading.columnconfigure(0, weight=1)
        ttk.Label(source_heading, text="Source documents", style="SurfaceTitle.TLabel").grid(row=0, column=0, sticky=tk.W)
        ttk.Label(source_heading, text="ZIP + PDF", style="Status.TLabel").grid(row=0, column=1, sticky=tk.E)
        ttk.Label(
            source_heading,
            text="Salary files, transfer receipts and insurance documents stay traceable through the run.",
            style="SurfaceBody.TLabel",
        ).grid(row=1, column=0, columnspan=2, sticky=tk.W, pady=(4, 0))

        listbox_frame = ttk.Frame(list_frame, style="Surface.TFrame")
        ...
        self.clear_btn = ttk.Button(button_frame, text="Clear", command=self.clear_all_files)
        ...
        # Progress + live log
        progress_frame = ttk.Frame(self.processing_tab, style="Card.TFrame", padding=16)
        ...
        status_row = ttk.Frame(progress_frame, style="Surface.TFrame")
        status_row.grid(row=1, column=0, sticky=(tk.W, tk.E))
        status_row.columnconfigure(0, weight=1)
        self.status_var = tk.StringVar()
        self.status_var.set("Ready")
        self.status_label = ttk.Label(status_row, textvariable=self.status_var, style="SurfaceBody.TLabel")
        ...
        self.log_frame = ttk.Frame(progress_frame, style="Surface.TFrame")
        ...
        # Shared by the data and chart views, whichever one is opened first.
        ...
        self._view_builders = {
            ...
            "Analytics Data Grid": self.create_analytics_grid_tab,
            "Analytics Graphs": self.create_analytics_tab,
            "Insurance": self.create_insurance_tab,
            "Employees": self.create_employees_tab,
            "Database": self.create_db_tab,
            "Settings": self.create_settings_tab,
        }
        self._built_views = {"Processing"}
        self._view_building = set()
    ...
    def _ensure_view_built(self, view_name):
        """Construct a view the first time it is opened."""
        builders = getattr(self, "_view_builders", {})
        built = getattr(self, "_built_views", set())
        building = getattr(self, "_view_building", set())
        builder = builders.get(view_name)
        if builder is None or view_name in built or view_name in building:
            return
        building.add(view_name)
        try:
            builder()
            built.add(view_name)
        finally:
            building.discard(view_name)
    ...
    def _sync_view_selector(self, _event=None):
        ...
        if label:
            self._ensure_view_built(label)
            self._set_active_view(label)
        # Let the selected tab receive a real size before charts draw. Drawing
        # during the tab-change callback gives Matplotlib a zero-sized canvas
        # and causes avoidable layout work and warnings.
        try:
            self.root.after_idle(self._refresh_active_view)
    ...
    def _export_tree_xlsx(self, tree, title="Export Data Grid"):
        ...
        columns, rows, meta_lines, totals_row = self._prepare_export_payload(tree)
        try:
            pandas_module = _load_pandas()
            df = pandas_module.DataFrame(rows, columns=columns)
            with pandas_module.ExcelWriter(
                path,
                engine="xlsxwriter",
    ...
    def _refresh_global_filters(self):
        if not self.db_config.get("enabled"):
            return
        now = time.monotonic()
        if getattr(self, "_global_years_loading", False):
            return
        if now - getattr(self, "_global_years_cached_at", 0) < 30:
            return
        self._global_years_loading = True
        ...
        self._run_async(
            "available-years",
            lambda: db_storage.fetch_available_years(self.db_config),
            _apply,
            _failed,
        )
    ...
    def _refresh_all_views(self):
        ...
        Hidden views rebuild from the current filter state when opened. The old
        cascade ran dozens of queries and rendered every chart after each key
        press, even though only one result could be visible.
        """
        self._render_filter_chips()
    ...
    def get_instructions_text(self):
        """One line of guidance; folder paths live beside the run status."""
        return "Add source files, review readiness, then create the summary and detail workbooks."
    ...
    def update_ui_state(self):
        ...
        if not deps_ready:
            self.warn_missing_dependencies()
        self._update_run_rail()
        self._refresh_setup_banner()

    def _update_run_rail(self):
        labels = getattr(self, "run_stage_labels", ())
        if not labels:
            return
        try:
        ...
        if progress >= 100:
            ...
            active = 1 if progress < 85 else 2
        elif self.zip_files:
            active = 1
        else:
            active = 0
        for index, label in enumerate(labels):
            label.configure(style="RunStepActive.TLabel" if index == active else "RunStep.TLabel")
