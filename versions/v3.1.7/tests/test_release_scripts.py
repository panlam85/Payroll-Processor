...
def test_bundle_builder_writes_version_marker_and_requires_tk():
    ...
    assert "APP_VERSION_SHORT = APP_VERSION" in builder


def test_bundle_relink_preserves_each_python_framework_dependency_path():
    builder_path = SCRIPTS_DIR / "create_simple_app.py"
    spec = importlib.util.spec_from_file_location("v316_bundle_builder", builder_path)
    builder = importlib.util.module_from_spec(spec)
    spec.loader.exec_module(builder)
    bundled = Path("/tmp/Test.app/Contents/Frameworks/Python.framework")

    assert builder._bundled_python_dependency_target(
        "/Library/Frameworks/Python.framework/Versions/3.13/lib/libssl.3.dylib",
        bundled,
    ) == bundled / "Versions/3.13/lib/libssl.3.dylib"
    assert builder._bundled_python_dependency_target(
        "/Library/Frameworks/Python.framework/Versions/3.13/Frameworks/Tk.framework/Tk",
        bundled,
    ) == bundled / "Versions/3.13/Frameworks/Tk.framework/Tk"
    assert builder._bundled_python_dependency_target("/usr/lib/libSystem.B.dylib", bundled) is None


def test_bundle_rsync_arguments_work_with_macos_system_rsync():
    builder = (SCRIPTS_DIR / "create_simple_app.py").read_text(encoding="utf-8")

    assert '"--no-xattrs"' not in builder
    assert '"--no-acls"' not in builder
    assert '"--progress"' not in builder
    assert 'for root, dirs, files in os.walk(app_path)' not in builder


def test_bundle_launcher_uses_the_bundled_python_standard_library():
    builder = (SCRIPTS_DIR / "create_simple_app.py").read_text(encoding="utf-8")
