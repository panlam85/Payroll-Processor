...
class PayrollProcessorGUI:
    ...
    def _open_employee_detail(self, employee_code=None, employee_name=None):
    ...
    def _build_data_grid(self, parent):
        ...
        export_btn.grid(row=0, column=4, padx=(10, 0), sticky=tk.W)

        prev_btn = ttk.Button(toolbar, text="◀ Prev", command=self._prev_grid_page)
        prev_btn.grid(row=0, column=5, padx=(16, 4), sticky=tk.W)
        next_btn = ttk.Button(toolbar, text="Next ▶", command=self._next_grid_page)
        next_btn.grid(row=0, column=6, padx=(0, 10), sticky=tk.W)

        ttk.Label(toolbar, text="Page", style="Body.TLabel").grid(row=0, column=7, padx=(0, 6))
        self.analytics_grid_page_label = ttk.Label(toolbar, textvariable=self.analytics_grid_page_var, style="Body.TLabel")
        self.analytics_grid_page_label.grid(row=0, column=8, padx=(0, 6), sticky=tk.W)
        ttk.Label(toolbar, textvariable=self.analytics_grid_total_var, style="Body.TLabel").grid(row=0, column=9, sticky=tk.W)

        ttk.Label(toolbar, text="Search uses top bar", style="Body.TLabel").grid(row=0, column=10, sticky=tk.W)

        self.analytics_grid_filters_frame = ttk.Frame(parent, style="App.TFrame")
        self.analytics_grid_filters_frame.grid(row=1, column=0, sticky=(tk.W, tk.E), pady=(0, 8))

        grid_frame = ttk.Frame(parent, style="App.TFrame")
        grid_frame.grid(row=2, column=0, sticky=(tk.W, tk.E, tk.N, tk.S))
    ...
    def _apply_grid_filters_from_cache(self):
        ...
        display_columns = self.analytics_grid_columns or columns
        ...
        filtered_rows = self._apply_grid_column_filters(filtered_rows, columns)
        if display_columns != self.analytics_grid_filter_columns:
            self._build_grid_filters(display_columns)
        indices = [columns.index(col) for col in display_columns if col in columns]
        display_rows = [tuple(row[idx] for idx in indices) for row in filtered_rows]
        self._reset_treeview(self.analytics_grid_tree, display_columns)
        ...
        if search or any(var.get().strip() for var in self.analytics_grid_filter_vars.values()):
            self.global_filter_status.set(f"Filtered ({len(display_rows)} rows)")
        else:
