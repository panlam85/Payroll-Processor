...
def fetch_monthly_summary(
    ...
    if document_type:
        ...
        params_doc = []
        if year is not None:
            conditions_doc.append("EXTRACT(YEAR FROM pe.payment_date) = %s")
            params_doc.append(year)
        if month is not None:
            conditions_doc.append("EXTRACT(MONTH FROM pe.payment_date) = %s")
            params_doc.append(month)
        conditions_doc.append("pe.document_type = %s")
        params_doc.append(document_type)
        where_clause = f"WHERE {' AND '.join(conditions_doc)}" if conditions_doc else ""
        query = f"""
            SELECT
...
def fetch_employer_costs_by_employee(
    ...
    params = []
    if year is not None:
        conditions.append("EXTRACT(YEAR FROM pe.payment_date) = %s")
        params.append(year)
    if month is not None:
        conditions.append("EXTRACT(MONTH FROM pe.payment_date) = %s")
        params.append(month)
    if document_type:
