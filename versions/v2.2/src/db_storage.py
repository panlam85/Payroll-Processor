...
def fetch_payroll_entry_count(
    ...
    """Return the total number of payroll entries for the analytics data grid."""
    _require_psycopg2()
    conditions = []
    params = []
    _append_date_range(conditions, params, "pe.payment_date", start_date, end_date)
    ...
    if search:
        conditions.append(
            "("
            "e.full_name ILIKE %s OR "
            "e.employee_code ILIKE %s OR "
            "pe.document_type::TEXT ILIKE %s OR "
            "d.source_pdf ILIKE %s OR "
            "pr.source_archive ILIKE %s"
            ")"
        )
