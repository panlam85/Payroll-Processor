...
class PayrollProcessorGUI:
    ...
    def __init__(self):
        ...
        self.analytics_grid_total_rows = 0
    ...
    def create_analytics_tab(self):
        ...
        self.analytics_tab.rowconfigure(3, weight=0)
        ...
        ttk.Label(header, text="Top Employees", style="Body.TLabel").grid(row=0, column=2, padx=(0, 6))
        ...
        top_spin.grid(row=0, column=3, padx=(0, 10))

        self.analytics_status_var = tk.StringVar(value="Ready to refresh.")
        ttk.Label(header, textvariable=self.analytics_status_var, style="Body.TLabel").grid(row=0, column=4, sticky=tk.W)
        ...
        for key, title in (
            ...
            if key == "monthly_employee":
                ...
                self.analytics_monthly_tab = frame
        ...
        footer = ttk.Frame(self.analytics_tab, style="App.TFrame")
        footer.grid(row=3, column=0, sticky=(tk.W, tk.E), pady=(8, 0))
        footer.columnconfigure(6, weight=1)

        self.analytics_footer_export_btn = ttk.Menubutton(footer, text="Export")
        ...
        self.analytics_footer_prev_btn = ttk.Button(footer, text="◀ Prev", command=self._prev_grid_page)
        self.analytics_footer_prev_btn.grid(row=0, column=1, padx=(16, 4), sticky=tk.W)
        self.analytics_footer_next_btn = ttk.Button(footer, text="Next ▶", command=self._next_grid_page)
        self.analytics_footer_next_btn.grid(row=0, column=2, padx=(0, 10), sticky=tk.W)

        ttk.Label(footer, text="Page", style="Body.TLabel").grid(row=0, column=3, padx=(0, 6))
        self.analytics_footer_page_label = ttk.Label(footer, textvariable=self.analytics_grid_page_var, style="Body.TLabel")
        self.analytics_footer_page_label.grid(row=0, column=4, padx=(0, 6), sticky=tk.W)
        self.analytics_footer_total_label = ttk.Label(footer, textvariable=self.analytics_grid_total_var, style="Body.TLabel")
        self.analytics_footer_total_label.grid(row=0, column=5, sticky=tk.W)

        self.analytics_notebook.bind("<<NotebookTabChanged>>", self._on_analytics_tab_change)

        self.analytics_heatmap_cbar = None
        self.analytics_heatmap_cax = None
        self.analytics_legend_map = {}
        self._update_analytics_footer_state()
    ...
    def _build_data_grid(self, parent):
        ...
        limit_spin.bind("<FocusOut>", self._on_grid_limit_change)
        ...
        ttk.Label(toolbar, text="Search uses top bar", style="Body.TLabel").grid(row=0, column=4, sticky=tk.W)
    ...
    def _load_grid_page(self, page_number):
        ...
        if search:
            self.global_filter_status.set(f"Filtered: {search} ({len(filtered_rows)} rows)")
        else:
    ...
    def export_grid_csv(self):
        if not hasattr(self, "analytics_grid_tree"):
            return
        self._export_tree_csv(self.analytics_grid_tree, title="Export Data Grid")
        ...
        try:
            ...
            with PdfPages(path) as pdf:
                ...
                for page_index in range(total_pages):
                    ...
                    ax.axis("off")
                    cell_text = [[str(val) for val in row] for row in page_rows]
                    table = ax.table(
                        cellText=cell_text,
                        colLabels=[str(col) for col in columns],
                        loc="center",
                        cellLoc="left",
                    )
    ...
    def _get_active_analytics_tree(self):
        if not hasattr(self, "analytics_notebook"):
            return None
        current = self.analytics_notebook.select()
    ...
    def _update_analytics_footer_state(self):
        current = self.analytics_notebook.select()
        is_grid = hasattr(self, "analytics_grid_tab") and str(self.analytics_grid_tab) == current
        is_data_tab = current in {
            str(getattr(self, "analytics_grid_tab", "")),
            str(getattr(self, "analytics_detail_tab", "")),
            str(getattr(self, "analytics_monthly_tab", "")),
        }
        export_state = "normal" if is_data_tab else "disabled"
        page_state = "normal" if is_grid else "disabled"
        if hasattr(self, "analytics_footer_export_btn"):
            self.analytics_footer_export_btn.configure(state=export_state)
        if hasattr(self, "analytics_footer_prev_btn"):
            self.analytics_footer_prev_btn.configure(state=page_state)
        if hasattr(self, "analytics_footer_next_btn"):
            self.analytics_footer_next_btn.configure(state=page_state)
        if not is_grid:
            self.analytics_grid_total_var.set("")

    def _on_analytics_tab_change(self, _event=None):
        self._update_analytics_footer_state()
    ...
    def refresh_dashboard(self):
        ...
        try:
            ...
            metrics = db_storage.fetch_dashboard_metrics(self.db_config, start_date=start_date, end_date=end_date)
