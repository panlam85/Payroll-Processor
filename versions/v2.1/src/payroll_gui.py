...
class PayrollProcessorGUI:
    ...
    def configure_styles(self):
        ...
        bg = "#1f1f1f" if is_dark else "#f5f5f0"
        surface = "#2a2a2a" if is_dark else "#ffffff"
        border = "#3a3a3a" if is_dark else "#d6d6d0"
        text_primary = "#f2f2f2" if is_dark else "#1a1a1a"
        text_secondary = "#c9c9c9" if is_dark else "#666666"
        ...
        style.configure("CardTitle.TLabel", background=surface, foreground=text_secondary, font=("Avenir Next", 11))
        style.configure("CardValue.TLabel", background=surface, foreground=text_primary, font=("Avenir Next", 18, "bold"))
        style.configure("Header.TLabel", background=bg, foreground=text_primary, font=("Avenir Next", 16, "bold"))
        style.configure("Section.TLabel", background=bg, foreground=text_primary, font=("Avenir Next", 13, "bold"))
        style.configure("Body.TLabel", background=bg, foreground=text_secondary, font=("Avenir Next", 11))
        style.configure("App.TLabelframe", background=bg)
        style.configure("App.TLabelframe.Label", background=bg, foreground=text_primary, font=("Avenir Next", 11, "bold"))
        style.configure("App.TNotebook", background=bg)
        style.configure("App.TNotebook.Tab", padding=(12, 6), font=("Avenir Next", 11))
        style.configure("Treeview", background=surface, fieldbackground=surface, foreground=text_primary, bordercolor=border)
        style.configure("Treeview.Heading", background=bg, foreground=text_primary, font=("Avenir Next", 11, "bold"))
    ...
    def create_analytics_tab(self):
        ...
        ttk.Label(header, text="Year", style="Body.TLabel").grid(row=0, column=2, padx=(0, 6))
        ...
        ttk.Label(header, text="Month", style="Body.TLabel").grid(row=0, column=4, padx=(0, 6))
        ...
        ttk.Label(header, text="Document", style="Body.TLabel").grid(row=0, column=6, padx=(0, 6))
        self.analytics_doc_type_var = tk.StringVar(value="All")
        self.analytics_doc_type_combo = ttk.Combobox(header, textvariable=self.analytics_doc_type_var, state="readonly", width=14)
        self.analytics_doc_type_combo["values"] = ["All", "salary", "bonus", "vacation_allowance", "unused_leave_compensation", "other"]
        self.analytics_doc_type_combo.grid(row=0, column=7, padx=(0, 10))

        ttk.Label(header, text="Top Employees", style="Body.TLabel").grid(row=0, column=8, padx=(0, 6))
        ...
        top_spin.grid(row=0, column=9, padx=(0, 10))

        self.analytics_status_var = tk.StringVar(value="Ready to refresh.")
        ttk.Label(header, textvariable=self.analytics_status_var, style="Body.TLabel").grid(row=0, column=10, sticky=tk.W)

        self.analytics_doc_type_combo.bind("<<ComboboxSelected>>", lambda _e: self.refresh_analytics())
    ...
    def refresh_analytics(self):
        ...
        try:
            ...
            doc_type_val = self.analytics_doc_type_var.get()
            ...
            total_net = 0.0
            total_employee_ins = 0.0
            total_employer_ins = 0.0
            for row in monthly_rows:
                total_net += float(row[3] or 0)
                total_employee_ins += float(row[4] or 0)
                total_employer_ins += float(row[5] or 0)
    ...
    def _plot_payment_heatmap(self, rows, year=None, month=None):
        ...
        if self.analytics_heatmap_cbar is not None:
            ...
            self.analytics_heatmap_cbar = None
        if self.analytics_heatmap_cax is not None:
            self.analytics_heatmap_cax.remove()
            self.analytics_heatmap_cax = None
    ...
    def _build_data_grid(self, parent):
        ...
        ttk.Label(toolbar, text="Search", style="Body.TLabel").grid(row=0, column=0, padx=(0, 6))
        self.analytics_grid_search_var = tk.StringVar(value="")
        search_entry = ttk.Entry(toolbar, textvariable=self.analytics_grid_search_var, width=24)
        search_entry.grid(row=0, column=1, padx=(0, 10))

        ttk.Label(toolbar, text="Limit", style="Body.TLabel").grid(row=0, column=2, padx=(0, 6))
        ...
        limit_spin.grid(row=0, column=3, padx=(0, 10))

        refresh_btn = ttk.Button(toolbar, text="Refresh Grid", command=self.refresh_data_grid)
        refresh_btn.grid(row=0, column=4, sticky=tk.W)
    ...
    def refresh_data_grid(self):
        ...
        doc_type_val = self.analytics_doc_type_var.get()
        document_type = None if doc_type_val == "All" else doc_type_val
        search = self.analytics_grid_search_var.get().strip()
        limit = int(self.analytics_grid_limit_var.get())
        ...
        self.analytics_grid_columns = columns
        self._reset_treeview(self.analytics_grid_tree, columns)
        self._populate_treeview(self.analytics_grid_tree, rows)
        for col in columns:
    ...
    def open_grid_column_selector(self):
        if not hasattr(self, "analytics_grid_columns"):
            return
        available = self.analytics_grid_columns
        if not available:
    ...
    def _filter_grid_columns(self):
        if not hasattr(self, "analytics_grid_columns"):
            return
        columns = self.analytics_grid_columns
        rows = [self.analytics_grid_tree.item(k, "values") for k in self.analytics_grid_tree.get_children("")]
        all_columns = self.analytics_grid_tree["columns"]
        if not all_columns:
            return
        indices = [all_columns.index(col) for col in columns if col in all_columns]
    ...
    def _on_grid_select(self, _event=None):
        ...
        self.analytics_notebook.select(self.analytics_detail_tree.master)
    ...
    def create_dashboard_tab(self):
        ...
        ttk.Label(header, text="Year", style="Body.TLabel").grid(row=0, column=2, padx=(0, 6))
        self.dashboard_year_var = tk.StringVar(value="All")
        self.dashboard_year_combo = ttk.Combobox(header, textvariable=self.dashboard_year_var, state="readonly", width=8)
        self.dashboard_year_combo.grid(row=0, column=3, padx=(0, 10))
        self.dashboard_year_combo.bind("<<ComboboxSelected>>", self._on_dashboard_year_change)
        self.dashboard_year_combo["values"] = ["All"]

        ttk.Label(header, text="Month", style="Body.TLabel").grid(row=0, column=4, padx=(0, 6))
        self.dashboard_month_var = tk.StringVar(value="All")
        self.dashboard_month_combo = ttk.Combobox(header, textvariable=self.dashboard_month_var, state="readonly", width=6)
        self.dashboard_month_combo.grid(row=0, column=5, padx=(0, 10))
        self.dashboard_month_combo["values"] = ["All"]

        self.dashboard_status_var = tk.StringVar(value="Ready.")
        ttk.Label(header, textvariable=self.dashboard_status_var, style="Body.TLabel").grid(row=0, column=6, sticky=tk.W)
        ...
        summary_frame.grid(row=2, column=0, sticky=(tk.W, tk.E, tk.N, tk.S))
        summary_frame.columnconfigure(0, weight=1)
        summary_frame.rowconfigure(0, weight=1)

        self.dashboard_summary_tree = ttk.Treeview(summary_frame, columns=(), show="headings")
        self.dashboard_summary_tree.grid(row=0, column=0, sticky=(tk.W, tk.E, tk.N, tk.S))

        y_scroll = ttk.Scrollbar(summary_frame, orient=tk.VERTICAL, command=self.dashboard_summary_tree.yview)
        y_scroll.grid(row=0, column=1, sticky=(tk.N, tk.S))
        x_scroll = ttk.Scrollbar(summary_frame, orient=tk.HORIZONTAL, command=self.dashboard_summary_tree.xview)
        x_scroll.grid(row=1, column=0, sticky=(tk.W, tk.E))
        self.dashboard_summary_tree.configure(yscrollcommand=y_scroll.set, xscrollcommand=x_scroll.set)

    def _refresh_dashboard_filters(self):
        years = db_storage.fetch_available_years(self.db_config)
        year_values = ["All"] + [str(year) for year in years]
        self.dashboard_year_combo["values"] = year_values
        if self.dashboard_year_var.get() not in year_values:
            self.dashboard_year_var.set(year_values[0] if year_values else "All")
        self._refresh_dashboard_months()

    def _refresh_dashboard_months(self):
        year_val = self.dashboard_year_var.get()
        if year_val == "All" or not year_val:
            self.dashboard_month_combo["values"] = ["All"]
            self.dashboard_month_var.set("All")
            return
        months = db_storage.fetch_available_months(self.db_config, int(year_val))
        month_values = ["All"] + [f"{month:02d}" for month in months]
        self.dashboard_month_combo["values"] = month_values
        if self.dashboard_month_var.get() not in month_values:
            self.dashboard_month_var.set(month_values[0] if month_values else "All")

    def _on_dashboard_year_change(self, _event=None):
        if not self.db_config.get("enabled"):
            return
        try:
            self._refresh_dashboard_months()
    ...
    def refresh_dashboard(self):
        ...
        try:
            ...
            year_val = self.dashboard_year_var.get()
            month_val = self.dashboard_month_var.get()
            year = int(year_val) if year_val and year_val != "All" else None
            month = int(month_val) if month_val and month_val != "All" else None

            metrics = db_storage.fetch_dashboard_metrics(self.db_config, year=year, month=month)
            ...
            self._reset_treeview(self.dashboard_summary_tree, columns)
            self._populate_treeview(self.dashboard_summary_tree, rows)
