...
class PayrollProcessorGUI:
    ...
    def create_analytics_tab(self):
        ...
        ttk.Label(header, text="Top Employees").grid(row=0, column=2, padx=(0, 6))
        ...
        ttk.Label(header, textvariable=self.analytics_status_var).grid(row=0, column=5, sticky=tk.W)
        ...
        self.analytics_monthly_ax = self.analytics_fig.add_subplot(2, 2, 1)
        self.analytics_insurance_ax = self.analytics_fig.add_subplot(2, 2, 2)
        self.analytics_employee_ax = self.analytics_fig.add_subplot(2, 1, 2)
    ...
    def refresh_analytics(self):
        ...
        try:
            monthly_rows = db_storage.fetch_monthly_summary(self.db_config)
            top_n = int(self.analytics_top_n_var.get())
            employee_rows = db_storage.fetch_employer_costs_by_employee(self.db_config, limit=top_n)
