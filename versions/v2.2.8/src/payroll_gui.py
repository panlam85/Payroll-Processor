...
class PayrollProcessorGUI:
    ...
    def __init__(self):
        ...
        self.show_db_tab_var = tk.BooleanVar(value=self.ui_prefs.get("show_database_tab", True))
        self.tooltips = []
        self.icons = {}
        self._load_icons()
        self.nav_history = []
    ...
    def _open_employee_detail(self, employee_code=None, employee_name=None, push_state=True):
        ...
        for col in detail_columns:
            self.analytics_detail_tree.heading(col, text=col, command=lambda c=col: self._sort_treeview(self.analytics_detail_tree, c, False))
        if self.analytics_grid_view_tab is not None:
            self.notebook.select(self.analytics_grid_view_tab)
    ...
    def _on_grid_click(self, event):
        ...
        try:
            ...
            if new_status:
                actual_value = self._get_grid_value(values, columns, "actual_payment_date")
                if self._is_empty_date(actual_value):
                    today = datetime.date.today()
                    db_storage.update_payroll_entry(self.db_config, entry_id, "actual_payment_date", today)
                    db_storage.append_audit_log(self.db_config, entry_id, "actual_payment_date", actual_value, today)
                    self.analytics_grid_tree.set(row_id, "actual_payment_date", today)
                    self._update_grid_cache(entry_id, "actual_payment_date", today)
        except Exception as exc:
    ...
    def _on_grid_double_click(self, event):
        ...
        if col_index < 0 or col_index >= len(display_columns):
            return
        col_name = display_columns[col_index]
        editable = {"document_type", "payment_date", "actual_payment_date", "basic_salary", "total_earnings", "net_pay"}
    ...
    def _validate_grid_edit(self, col_name, value):
        ...
        if col_name in {"payment_date", "actual_payment_date"}:
    ...
    def _apply_ui_prefs(self):
        ...
        if "show_database_tab" in prefs:
            self.show_db_tab_var.set(bool(prefs.get("show_database_tab")))
    ...
    def _open_grid_edit_modal(self):
        ...
        original = {
            ...
            "actual_payment_date": current_value("actual_payment_date") or "",
        ...
        actual_date_var = tk.StringVar(value=str(original["actual_payment_date"]))
        ...
        ttk.Label(frame, text="Actual Payment Date (YYYY-MM-DD)", style="Body.TLabel").grid(row=3, column=0, sticky=tk.W, pady=(0, 6))
        ...
        def on_save():
            updates = {
                ...
                "actual_payment_date": actual_date_var.get().strip(),
                "basic_salary": basic_var.get().strip(),
                "total_earnings": total_var.get().strip(),
                "net_pay": net_var.get().strip(),
            }
            if updates["paid_status"].strip().lower() in {"yes", "true", "1", "paid"} and not updates["actual_payment_date"].strip():
                updates["actual_payment_date"] = datetime.date.today().isoformat()
            normalized = {}
    ...
    def refresh_monthly_employee_summary(self):
        ...
        try:
            ...
            for col in columns:
                self.analytics_monthly_tree.heading(col, text=col, command=lambda c=col: self._sort_treeview(self.analytics_monthly_tree, c, False))
            timestamp = datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S")
    ...
    def get_instructions_text(self):
        ...
        if DRAG_DROP_AVAILABLE:
            return ("Drag and drop ZIP or PDF files containing payroll data, "
                    ...
                    "Update the output folder from Settings → Output Folder.")
    ...
    def create_menu(self):
        ...
        settings_menu.add_checkbutton(
            label="Show Database Tab",
            variable=self.show_db_tab_var,
            command=self._toggle_database_tab,
        )
        settings_menu.add_command(label="Output Folder", command=self.choose_output_folder)
        settings_menu.add_command(label="Appearance", command=self.open_theme_settings)
    ...
    def show_help(self):
        ...
        messagebox.showinfo(
            ...
            "Output folder can be changed from Settings → Output Folder."
        )
    ...
    def process_files(self, summary_output, detail_output):
        ...
        try:
            ...
            with tempfile.TemporaryDirectory() as temp_dir:
                ...
                for i, file_path in enumerate(self.zip_files):
