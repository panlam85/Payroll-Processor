...
class PayrollProcessorGUI:
    ...
    def create_widgets(self):
        ...
        self.output_location_var = tk.StringVar(
            value=f"Reports folder: {self.report_dir}"
        )
        self.output_location_label = ttk.Label(
            progress_frame, textvariable=self.output_location_var, font=("Arial", 9)
        )
    ...
    def generate_reports(self):
        ...
        # Determine automatic output location
        timestamp = datetime.datetime.now().strftime('%Y%m%d_%H%M%S')
        output_file = self.report_dir / f"employee_reports_{timestamp}.xlsx"
        output_file = str(output_file)
        self.last_output_path = output_file
        self.output_location_var.set(f"Saving to: {output_file}")
    ...
    def process_files(self, output_file):
        ...
        try:
            ...
            with tempfile.TemporaryDirectory() as temp_dir:
                ...
                self.root.after(0, lambda: self.output_location_var.set(f"Last report: {output_file}"))
