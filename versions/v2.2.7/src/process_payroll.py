...
def _archive_pdf_for_entry(
    ...
    code = _sanitize_segment(entry.get("EmployeeCode") or "")
    name_suffix = f"_{code}" if code else ""
    if merge_if_exists:
        dest_name = f"{base}{name_suffix}.pdf"
    else:
        file_suffix = f"_{suffix}" if suffix else ""
        dest_name = f"{base}{name_suffix}{file_suffix}.pdf"
