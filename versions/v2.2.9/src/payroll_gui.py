...
class PayrollProcessorGUI:
    ...
    def _load_icons(self):
        ...
        icons = {}
        for name in ("refresh", "export", "database"):
    ...
    def create_analytics_grid_tab(self):
        ...
        self.analytics_footer_undo_btn = ttk.Button(footer, text="Undo", command=self._undo_last_edit)
    ...
    def _build_detail_tab(self, parent):
        ...
        columns_btn = ttk.Button(header, text="Columns...", command=self.open_detail_column_selector)
            ...
            with PdfPages(path) as pdf:
                ...
                for page_index in range(total_pages):
                    ...
                    table = ax.table(
                        ...
                        loc="center",
                        cellLoc="left",
                        bbox=[0.02, 0.08, 0.96, 0.74] if meta_lines else [0.02, 0.02, 0.96, 0.9],
                    )
    ...
    def _should_total_column(self, column_name):
        ...
        skip_tokens = ("date", "id", "code", "month", "year", "type", "name", "source")
    ...
    def _build_monthly_employee_tab(self, parent):
        ...
        columns_btn = ttk.Button(toolbar, text="Columns...", command=self.open_monthly_column_selector)
        columns_btn.grid(row=0, column=1, padx=(0, 10))
    ...
    def create_settings_tab(self):
        ...
        ttk.Label(storage_frame, text="Reports folder:", style="Body.TLabel").grid(row=0, column=0, sticky=tk.W)
        ...
        ttk.Button(storage_frame, text="Change…", command=self.choose_output_folder).grid(row=0, column=2, padx=(8, 0))

        ttk.Label(storage_frame, text="PDF archive folder:", style="Body.TLabel").grid(row=1, column=0, sticky=tk.W, pady=(6, 0))
        ...
        ttk.Button(storage_frame, text="Change…", command=self.choose_pdf_archive_folder).grid(row=1, column=2, padx=(8, 0), pady=(6, 0))
        ...
        ttk.Label(watch_frame, text="Folder:", style="Body.TLabel").grid(row=0, column=0, sticky=tk.W)
        ...
        ttk.Button(watch_frame, text="Change…", command=self.choose_watch_folder).grid(row=0, column=2, padx=(8, 0))
        ttk.Checkbutton(
            watch_frame,
        ...
        ttk.Label(watch_frame, text="Interval (sec)", style="Body.TLabel").grid(row=1, column=1, sticky=tk.W, pady=(6, 0))
        interval_spin = ttk.Spinbox(watch_frame, from_=2, to=120, textvariable=self.watch_interval_var, width=6, command=self._toggle_watch_folder)
        interval_spin.grid(row=1, column=2, sticky=tk.W, pady=(6, 0))
        ...
        ttk.Button(db_frame, text="Database Settings…", command=self.open_db_settings).grid(row=0, column=0, padx=(0, 10))
        ttk.Label(db_frame, text="Backups and exports", style="Body.TLabel").grid(row=0, column=1, sticky=tk.W)
        ttk.Button(db_frame, text="Backup Database…", command=self.backup_database).grid(row=1, column=0, pady=(8, 0))
        ...
        ttk.Button(db_frame, text="Export Data (CSV)…", command=self.export_database_csv).grid(row=1, column=2, padx=(10, 0), pady=(8, 0), sticky=tk.W)
        ttk.Checkbutton(
            db_frame,
        ...
        ttk.Label(backup_frame, text="Backup folder:", style="Body.TLabel").grid(row=0, column=0, sticky=tk.W)
        ...
        ttk.Button(backup_frame, text="Change…", command=self.choose_backup_folder).grid(row=0, column=2, padx=(8, 0))
        ttk.Checkbutton(
            backup_frame,
            text="Auto backup",
        ...
        ttk.Button(backup_frame, text="Run Backup Now", command=self.run_total_backup).grid(row=2, column=0, pady=(8, 0))
        ...
        ttk.Button(appearance_frame, text="Appearance…", command=self.open_theme_settings).grid(row=0, column=0, padx=(0, 10))
        ttk.Checkbutton(
            appearance_frame,
                ...
                for i, file_path in enumerate(file_paths):
                    ...
                    try:
                        ...
                        if file_path.lower().endswith(".zip"):
                            df = process_payroll.process_zip(file_path, temp_dir, archive_root=str(self.archive_dir))
