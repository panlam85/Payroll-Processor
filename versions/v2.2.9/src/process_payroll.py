...
def parse_transfer_receipt(pdf_path: str):
    ...
    try:
        ...
        return None
    if "Κωδικός Συναλλαγής" not in text and "Μεταφορά σε τρίτο" not in text:
        return None
    name_match = re.search(r"Κύριος\s+Δικαιούχος:\s*(.+)", text)
    amount_match = re.search(r"Ποσό:\s*([0-9\.,]+)", text)
    date_match = re.search(r"Εκτέλεση\s+Στις\s+(\d{1,2}/\d{1,2}/\d{4})", text)
    ...
    try:
        ...
        return None
    return {"employee_name": name, "amount": amount, "paid_date": paid_date}
...
def _archive_pdf_for_entry(
    ...
    dest_name = _build_archive_filename(entry, suffix=suffix, merge_if_exists=merge_if_exists)
...
def _build_archive_filename(entry: Dict[str, str], suffix: str = "", merge_if_exists: bool = True) -> str:
    date_str = entry.get("Date") or ""
    date_part = "unknown"
    try:
        parsed = datetime.datetime.strptime(date_str, "%d/%m/%Y")
    ...
    base = f"{date_part}_{name_part}_{doc_part}"
    if merge_if_exists:
        return f"{base}.pdf"
...
def process_zip(zip_path: str, temp_root: str, archive_root: str = None) -> pd.DataFrame:
                ...
                if archive_root and slips:
                    if len(slips) == 1:
                        ...
                        if key not in archived_keys:
                            _archive_pdf_for_entry(archive_root, file_path, slips[0])
                        ...
                        if page_files and len(page_files) >= len(slips):
                            for index, entry in enumerate(slips):
                                ...
                                _archive_pdf_for_entry(
                                    ...
                                    page_files[index],
                                    entry,
                                    suffix=f"page{index + 1}",
                                    merge_if_exists=True,
                                )
                                archived_keys.add(key)
                        else:
                            for index, entry in enumerate(slips):
                                key = (file_path, entry.get("EmployeeCode") or entry.get("EmployeeName"), f"entry{index + 1}")
                                if key in archived_keys:
                                    continue
                                _archive_pdf_for_entry(
                                    archive_root,
...
def process_pdf_file(pdf_path: str, temp_root: str, archive_root: str = None) -> pd.DataFrame:
    """Parse a single PDF and return extracted records as a DataFrame."""
    records: List[Dict[str, str]] = []
    archived_keys = set()
    ...
    if archive_root and slips:
        if len(slips) == 1:
            ...
            if key not in archived_keys:
                _archive_pdf_for_entry(archive_root, pdf_path, slips[0])
                archived_keys.add(key)
        else:
            split_dir = tempfile.mkdtemp(prefix="pages_", dir=temp_root)
            ...
            if page_files and len(page_files) >= len(slips):
                ...
                for index, entry in enumerate(slips):
                    key = (pdf_path, entry.get("EmployeeCode") or entry.get("EmployeeName"), f"entry{index + 1}")
                    if key in archived_keys:
                        continue
                    _archive_pdf_for_entry(
                        archive_root,
