...
- `versions/v1.0/` … `versions/v3.1.4/` – frozen historical iterations, retained for reference
- `versions/v3.1.5/` – **active** source, scripts, docs and resources
- Root-level helper scripts (`launch_gui.sh`, `run_dev.sh`, `payroll_cli.sh`, `create_simple_app.py`, `create_simple_installer.sh`) delegate to `versions/v3.1.5` so you always run the latest code while keeping older releases intact.
...
### **Core Application Files (v3.1.5):**
- `versions/v3.1.5/src/payroll_gui.py` – GUI shell, sidebar views (Dashboard, Analytics, Insurance, Employees, Processing, Database, Settings), menus, and report orchestration
- `versions/v3.1.5/src/process_payroll.py` – PDF extraction and payroll/insurance/receipt parsing
- `versions/v3.1.5/src/db_storage.py` – PostgreSQL schema, migrations, imports, exports, and backup/restore
- `versions/v3.1.5/src/create_employee_reports.py` – Excel report generation (per-employee workbook + analytical detail workbook)
- `versions/v3.1.5/src/payroll_cli.py` – CLI entry point for headless runs and run-ledger queries
- `versions/v3.1.5/resources/app_icon.icns` – Custom application icon
- `versions/v3.1.5/requirements.txt` – Python dependencies for the GUI and bundler
- `versions/v3.1.5/scripts/*.sh|py` – Launchers, development runner, bundle and installer builders

### **Standalone Mac App:**
- `dist/Payroll Processor.app` - Bundle generated from v3.1.5 sources
...
### **Development Tools:**
- `launch_gui.sh` – Wrapper that selects a Tk-capable Python, maintains a requirement-hashed runtime cache inside `versions/v3.1.5`, and launches the GUI
...
versions/v3.1.5/.venv/bin/python -m pytest -q --cov --cov-config=.coveragerc
```
Coverage gate is 65%; the parsing/storage/CLI core currently sits at 94.47%. `payroll_gui.py` is excluded from the gate; its display-independent helpers are covered by `tests/test_payroll_gui_helpers.py`.
...
1. **Install the Application:**
   ...
   ./create_simple_installer.sh     # builds releases/v3.1.5/ artifacts
   ```
   Then install:
   ```bash
   # Option 1: Use the installer
   open releases/v3.1.5/PayrollProcessor_Installer/
   # Double-click "Install Payroll Processor.command"

   # Option 2: Mount the DMG directly
   open releases/v3.1.5/PayrollProcessor_v3.1.5_macOS.dmg
...
```bash
# Quick development testing (uses versions/v3.1.5 automatically)
./run_dev.sh

# Build/refresh the macOS bundle in dist/
./create_simple_app.py

# Generate installer artifacts in releases/v3.1.5/
...
payment processor/
├── versions/
│   ├── v1.0/ … v3.1.4/    # Older code paths retained for reference
│   └── v3.1.5/            # Active codebase (src/scripts/resources/docs/tools/tests)
├── dist/                  # Generated “Payroll Processor.app” (gitignored)
├── releases/              # Distributables: ZIP/DMG/Installer (gitignored)
├── launch_gui.sh          # Wrapper → versions/v3.1.5/scripts/launch_gui.sh
├── run_dev.sh             # Wrapper → versions/v3.1.5/scripts/run_dev.sh
├── payroll_cli.sh         # Wrapper → versions/v3.1.5/scripts/payroll_cli.sh
├── create_simple_app.py   # Wrapper → versions/v3.1.5/scripts/create_simple_app.py
├── create_simple_installer.sh  # Wrapper → versions/v3.1.5/scripts/create_simple_installer.sh
